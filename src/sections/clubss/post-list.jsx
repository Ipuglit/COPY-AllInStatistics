  ...
  return (
    <Stack direction="row" alignItems="center" spacing={3} key={i.id} sx={{ width: '100%' }}>
      ...
      <Box
        component="img"
        alt={i.name}
        src={i.imageFull ? i.imageFull : "public/images/clubs/default.png"}
        sx={{ width: 48, height: 48, borderRadius: 1.5, flexShrink: 0 }}
      />

      <Box sx={{ minWidth: 275, flexGrow: 1, '@media (max-width: 600px)':{minWidth: 115, flexGrow: 1,} }} >

        <Typography variant="subtitle1" fontSize="small" sx={{ color: 'text.secondary' }} noWrap style={{ marginBottom: '-5px' }}>
          <span style={{fontSize:"11px"}}>{i.appName}</span>
        </Typography>
        ...
        <Typography variant="body2" sx={{ color: 'text.secondary' }} noWrap style={{ marginTop: '-5px' }}>
          ID: {i.idd}
        </Typography>

        <Typography variant="subtitle1" fontSize="small" sx={{ color: 'text.secondary' }} noWrap style={{ marginTop: '-8px' }}>
        <span style={{fontSize:"10px"}}>{i.type == "PRIVATE" ? "PRIVATE" : i.type == "UNION" ? i.unionName : "NONE"}</span>
        </Typography>

        <Typography variant="subtitle1" fontSize="small" sx={{ color: 'text.secondary' }} noWrap style={{ marginTop: '-8px' }}>
        <span style={{fontSize:"10px"}}>{i.usersActive == 0 ? "No accounts" : i.usersActive == 1 ? "1 account" : i.usersActive+" accounts"}</span>
        </Typography>
