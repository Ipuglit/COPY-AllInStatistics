...
export default function Charts_Users({ onTitle, onFiltered, subheader, chart, sublabel, barWidth, ...other }) {

  const [limit, setlimit]                    = useState(5);
  const [rawFETCH, setrawFETCH]              = useState({...Cls.RawDefault, LIMIT: limit});
  const [rawREFRESH, setrawREFRESH]          = useState(1);
    
    const fetchDATA        = RawFetch(rawFETCH,rawREFRESH,'userslist')

    const chartDATA    = (Fnc.isZERO(fetchDATA.data) ? fetchDATA.data  : []).reduce((a, i) => {
                                a.name.push(i.userFirstname ? i.userName.toUpperCase() : i.userNick.toUpperCase());
