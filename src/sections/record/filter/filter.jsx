...
export default function FilterThis({LOAD,DATA,RECEIVE,RETURN}){
    ...
    const optApps       = dropItems(DATA,'appName','appID')?.filter((i, index, s) => s.findIndex((t) => t?.label === i?.label) === index )    
    const optClubs      = dropItems(DATA,'clubName','clubID')?.filter((i, index, s) => s.findIndex((t) => t?.label === i?.label) === index )        
    const optAccts      = dropItems(DATA,'playerID','playerID')?.filter((i, index, s) => s.findIndex((t) => t?.label === i?.label) === index )         
    const optLine       = [...dropItems(DATA,'uplineID','uplineID'), ...dropItems(DATA,'agencyID','agencyID'), dropItems(DATA,'downlineID','downlineID')]
    const optLines      = optLine?.filter((i, index, s) => { 
                                                                if( i != null && i!='' && i != undefined){ 
                                                                            return s.findIndex((t) => t?.label === i?.label) === index
                                                                }
    ...
    const onSubmitting=()=>{
        RETURN({
                    ...
                    PLAYER:         getValue(onPlayer),
                    LINES:          getValue(onLines), 
  ...
  return (
    <>
        ...
        <Button variant={available ? 'contained' : 'outlined'} 
                  ...
                  sx={{...Cls.buttonClass(available ? 'contained' : 'outlined','violet'), minWidth:onMobile ? '' : '125px',fontSize: onMobile ? '12px' : '',borderRadius:'0px',}} 
                  size='small'>
                    {
                      onMobile
                      ?
            ...
            <Grid container columns={{ xs: 12, sm: 12, md: 12 }}>
                ...
                <Grid item xs={12} sm={12} md={12} padding={1}>
                    <Autocomplete   multiple={true}
                                    ...
                                    getOptionLabel={(option) => option?.label || ''}
                                    isOptionEqualToValue={isOptEqualTo}
                                    onChange={(event, newValues)=>setonLines(newValues)}
                                    renderTags={(value, getTagProps)=>renderTAGS(value, getTagProps,onLines,setonLines)}
                                    renderOption={renderOpt}
                                    renderInput={(params) => 
                                    <TextField {...params} 
                                        size='small'
                                        sx={textSX}
                                        autoComplete='off'
                                        label="Select line ID/s" />
                                    }
                                    />
                </Grid>

            </Grid>
            {Fnc.JSONS(optLines,false)}
        </DialogContent>
              ...
              { 
                !onSubmitLoad ? 
                  ...
                  <>
                    ...
                    <Button onClick={()=>onReset()} sx={{borderRadius:'0',width:'50%',fontSize: onMobile ? '11px' : ''}} variant='outlined' loading='true' >
                        RESET
                    </Button>
                  </>
                :
