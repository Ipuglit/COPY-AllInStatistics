...
import {FormControlLabel,Checkbox,Autocomplete,TextField,DialogContent,DialogContentText,DialogActions,Button,Box, Divider} from '@mui/material';
...
export default function DateRange({ onClose, onData, onItems, onReturn }) {
    ...
    const onSubmit =(i)=>{
        onReturn({
                ...
                row:            itemx.ROW, 
                what:           onData.name, 
                subDateOpen:    itemx.DATEOPENNED, 
                subDateClose:   itemx.DATECLOSED, 
  ...
  return (
      ...
      <DemoContainer components={['DatePicker']}>
        ...
        <DatePicker label="Date closed"
          ...
          <FormControlLabel
            ...
            label={<span style={{fontSize: onMobile ? '10px' : '11px', color:checkDate ? 'violet' : ''}}>Apply to all same opening and closing dates</span>}
          />
          ...
          sx={{marginTop:'-10px'}}
            ...
            label={<span style={{fontSize: onMobile ? '10px' : '11px', color:checkClub ? 'violet' : ''}}>Apply to all club '{itemx?.CLUBNAME}'</span>}
          />
            </>
          }

        {
            ...
            sx={{marginTop:'-10px'}}
              ...
              label={<span style={{fontSize: onMobile ? '10px' : '11px', color:checkPlayer ? 'violet' : ''}}>Apply to all player ID: '{itemx?.PLAYERID}'</span>}
            />
            </>
    ...
    <DialogActions style={{paddingBottom:'30px',padding:'30px',display: 'flex', justifyContent: 'center', marginTop:'-30px'}}>
            ...
            {
              Fnc.dateDash(itemx?.DATEOPENNED) == Fnc.dateDash(openned) && Fnc.dateDash(itemx?.DATECLOSED) == Fnc.dateDash(closed) || (Fnc.isNull(openned)) || (Fnc.isNull(closed)) || ( openned?.isAfter(closed) ) || ( openned?.isAfter(today) ) || ( closed?.isAfter(today) )
              ?
              null
              :
