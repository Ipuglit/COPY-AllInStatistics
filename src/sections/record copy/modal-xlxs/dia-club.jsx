...
import {FormControlLabel,Checkbox,Autocomplete,TextField,DialogContent,DialogContentText,DialogActions,Button,Box,Typography} from '@mui/material';
...
export default function Club({ onClose, onData, onItems, onReturn }) {
  ...
  const onMobile    = Fnc.OnMobile()
    ...
    const listDDown = onItems.CLUBS?.data ? onItems.CLUBS?.data : onItems.CLUBS
    ...
    const onSubmit =(i)=>{
      onReturn({...value, row: itemx.ROW, what: onData.name, apply: checked, sub: itemx.CLUBSUB, club: itemx.CLUB})
    }
    ...
    <>
        ...
        <DialogContent>
          ...
          <DialogContentText component="section">
            ...
            <FormControlLabel
              ...
              label={<Typography variant="body1" style={{fontSize: onMobile ? '10px' : '12px', color:checked ? 'violet' : ''}}>Apply to all club '{itemx.CLUBSUB ? itemx.CLUBSUB : itemx.CLUBNAME}'</Typography >}
            />
            }
        ...
        <DialogActions style={{paddingBottom:'30px',padding:'30px',display: 'flex', justifyContent: 'center', marginTop:'-30px'}}>
            ...
            <Button variant='outlined' 
                    disabled={itemx?.CLUBID > 0 ? false : true}
                    onClick={onClose} 
